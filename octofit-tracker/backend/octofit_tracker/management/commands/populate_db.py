...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        octo_models.Activity.objects.create(user=spiderman, type='cycle', duration=45, calories=400)
        ...
        # Crear leaderboard
        octo_models.Leaderboard.objects.create(user=ironman, points=1000)
        octo_models.Leaderboard.objects.create(user=spiderman, points=900)
        octo_models.Leaderboard.objects.create(user=batman, points=1100)
        octo_models.Leaderboard.objects.create(user=superman, points=1200)
